...
export default async function pledge(
  ...
  if (chainId == 31337) {
    ...
    await mockToken.connect(pledger1).mint(pledger1, amount);
    await mockToken.connect(pledger1).approve(crowdfund.address, amount);

    await crowdfund.connect(pledger1).pledge(campaignId, amount);
